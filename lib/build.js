...
import { formatDate, mkdir } from "./helpers.js";
...
const render = (srcFolder, publicFolder, file) => {
  ...
  fs.writeFileSync(path.join(publicFolder, fileNameHtml), rendered, {
    flag: "w+",
  });
...
export const parseContent = () => {
  return getAllFiles(path.join(app.src, app.content), [], '/')
    .filter(o => o != null) // make sure there are no undefined items
};
...
export const generateBlogListPage = (posts) => {
  ...
  fs.writeFileSync(path.join(blogListDirectory, 'index.html'), rendered, {
    flag: "w+",
  });
};
